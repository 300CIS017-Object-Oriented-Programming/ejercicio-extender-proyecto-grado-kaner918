    ...
    def exportar_acta(self, st, controlador, acta_seleccionada):
        ...
        for acta in controlador.actas:
            if acta.autor == acta_seleccionada:
                ...
                self.pdf.multi_cell(185, 6, txt=f"Trabajo de grado denominado: {acta.nombre_trabajo}",ln=1, align='L')
                ...
                for criterio in acta.criterios:
                    self.pdf.set_font("times", 'B', size=12)
                    self.pdf.multi_cell(185, 10, txt=f"{numero}. {criterio.descripcion}", ln=1, align='L')
                    self.pdf.set_font("times", size=12)
                    self.pdf.cell(145, 7, txt=f"Calificación parcial: {round(criterio.nota, 2)}", ln=1, align='L')
                    self.pdf.cell(100, 7, txt=f"Ponderación: {criterio.porcentaje * 100}%", ln=1, align='L')
                    self.pdf.multi_cell(185, 7, txt=f"Observación: {criterio.observacion}", ln=1, align='L')
                    self.pdf.multi_cell(185, 7, txt=f"Observación adiccional: {criterio.observaciones_adicionales}", ln=1, align='L')
                    self.pdf.multi_cell(185, 7, txt=f"Restricciones: {criterio.restricciones}", ln=1, align='L')
                ...
                self.pdf.set_font("times", size=12)
                self.pdf.multi_cell(185, 5, txt="Observaciones adicionales: ", ln=1, align='L')
                self.pdf.multi_cell(185, 5,
                               txt="_____________________________________________________________________________________",
                               ln=1,align='L')
                self.pdf.multi_cell(185, 5,
                ...
                self.pdf.cell(200, 30, txt="________________________________   ________________________________", ln=2,
                         align='C')
                self.pdf.cell(200, 5, txt="Firma Jurado 1                                      Firma Jurado 2 ", ln=2, align='C')
